...
let header = ["Type", "Buy Amount", "Buy Currency", "Sell Amount", "Sell Currency", "Fee Amount", "Fee Currency", "Exchange", "Trade Group", "Comment", "Date"];
var parser = parse({
  delimiter: ","
}, function (err, data) {
...
var parser = parse({
  ...
  fs.open(`output/poloniexDepositHistory.csv`, "w", (err, fd) => {
    ...
    data.forEach(d => {
      if (d[2] !== "Amount") withdrawOut.push(["Deposit", d[2], d[1], "", "", "", "", "Poloniex", "", d[3], d[0]]);
    });
    let contents = "";
...
var parser = parse({
  ...
  fs.open(`output/poloniexWithdrawHistory.csv`, "w", (err, fd) => {
    ...
    data.forEach(d => {
      if (d[2] !== "Amount") depositOut.push(["Withdrawal", "", "", d[2], d[1], "", "", "Poloniex", "", d[3], d[0]]);
    });
    let contents = "";
