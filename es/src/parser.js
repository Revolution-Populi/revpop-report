...
function trackMovements(asset, amount, type, timestamp) {
  ...
  if (!movementTypes[asset][type]) movementTypes[asset][type] = {
    deposit: [],
    withdrawal: []
  };
...
function addOutputEntry(output, type, buy, sell, fee, date, opType, comment, tradeGroup) {
  ...
  output.push([type, utils.printAmount(buy), buy.currency, utils.printAmount(sell), sell.currency, utils.printAmount(fee), fee.currency, "BTS-DEX", tradeGroup || "", comment || "", date]);
  return output;
}
  ...
  for (var i = recordKeys.length - 1; i >= 0; i--) {
    ...
    switch (type) {
      case "fill_order":
        ...
        if (!!previous && t1.isSame(t0, "day") && previous.data.pays.asset_id === data.pays.asset_id && previous.data.receives.asset_id === data.receives.asset_id) {
...
function parseData(recordData, accountId, accountName) {
  let out = [];
  out.push(["Type", "Buy Amount", "Buy Currency", "Sell Amount", "Sell Currency", "Fee Amount", "Fee Currency", "Exchange", "Trade Group", "Comment", "Date"]);
  let typeCounts = {};
  ...
  for (let trx_id of Object.keys(recordData)) {
    ...
    switch (type) {
      case "vesting_balance_withdraw":
        ...
        out = addOutputEntry(out, data.owner === "1.2.30665" && vestingFunds.amount > 10000 ? "Income" : "Deposit", vestingFunds, null, fee, // dev.bitsharesblocks
        timestamp, type, `${accountName} : Vesting balance withdraw`);
        incrementType(type);
        break;

      case "balance_claim":
        let balanceClaimFunds = utils.parseCurrency(data.total_claimed);
        out = addOutputEntry(out, "Deposit", balanceClaimFunds, null, null, timestamp, type, `${accountName} : Balance claim`);
        incrementType(type);
        break;
      ...
      case "transfer":
        ...
        if (data.to == accountId) {
          ...
          out = addOutputEntry(out, data.to === "1.2.391938" && data.from === "1.2.381086" ? "Income" : "Deposit", funds, null, null, // pay.svk and bitshares-ui
          timestamp, type, `${accountName} : From ${data.from}`);
        } else {
          out = addOutputEntry(out, "Withdrawal", null, funds, fee, timestamp, type, `${accountName}: To ${data.to}`);
        }
      ...
      case "fill_order":
        ...
        out = addOutputEntry(out, "Trade", boughtFunds, soldFunds, fee, timestamp, type);
        incrementType(type);
        break;
      ...
      case "asset_issue":
        {
          ...
          if (data.issue_to_account === accountId) {
            out = addOutputEntry(out, "Deposit", issuedFunds, null, fee, timestamp, type, `${accountName} : Issued to account`);
          }
      ...
      case "call_order_update":
        ...
        if (fee.amount > 0) {
          out = addOutputEntry(out, "Withdrawal", null, fee, null, timestamp, type, `${type} fee`);
          incrementType(type);
        }
      ...
      case "asset_fund_fee_pool":
        {
          ...
          out = addOutputEntry(out, "Withdrawal", null, fundFunds, fee, timestamp, type, `${type}`);
          incrementType(type);
          break;
        }
