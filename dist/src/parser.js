...
function addOutputEntry(output, type, buy, sell, fee, date, opType, comment, tradeGroup) {
  ...
  output.push([type, utils.printAmount(buy), buy.currency, utils.printAmount(sell), sell.currency, utils.printAmount(fee), fee.currency, "RVP-DEX", tradeGroup || "", comment || "", date]);
  return output;
}
  ...
  for (let trx_id of Object.keys(recordData)) {
    ...
    switch (type) {
      case "vesting_balance_withdraw":
        ...
        out = addOutputEntry(out, data.owner === "1.2.30665" && vestingFunds.amount > 10000 ? "Income" : "Deposit", vestingFunds, null, fee, timestamp, type, `${accountName} : Vesting balance withdraw`);
        incrementType(type);
        break;
      ...
      case "transfer":
        ...
        if (data.to == accountId) {
          ...
          out = addOutputEntry(out, data.to === "1.2.391938" && data.from === "1.2.381086" ? "Income" : "Deposit", funds, null, null, timestamp, type, `${accountName} : From ${data.from}`);
        } else {
