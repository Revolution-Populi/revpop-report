...
module.exports = function (isBrowser) {
  if (isBrowser) fetchClient = fetch;else {
    fetchClient = require("node-fetch");
  }

  function getAccountHistoryES(account_id, limit, start, esNode = "https://testnet.revolutionpopuli.com/wrapper/elasticsearch") {
    ...
    return new Promise((resolve, reject) => {
      fetchClient(`${esNode}/get_account_history?account_id=${account_id}&from_=${start}&size=${limit}&sort_by=block_data.block_time&type=data&agg_field=operation_type`).then(res => res.json()).then(result => {
        let ops = result.map(r => {
          if ("amount_" in r.operation_history.op_object) {
            r.operation_history.op_object.amount = r.operation_history.op_object.amount_;
          }
  ...
  function getAccountHistory(account_id, stop, limit, start) {
    return new Promise((resolve, reject) => {
      wsLib.Apis.instance().history_api().exec("get_account_history", [account_id, stop, limit, start]).then(operations => {
        resolve(operations);
      }).catch(reject);
    });
  }
