...
function connect() {
  return new Promise(resolve => {
    wsLib.Apis.instance(config.apiNode, true).init_promise.then(res => {
...
function getBlockTime(block) {
  return new Promise((resolve, reject) => {
    ...
    wsLib.Apis.instance().db_api().exec("get_block", [block]).then(result => {
...
function getAssetData(asset) {
  return new Promise((resolve, reject) => {
    ...
    FetchChain("getObject", asset, undefined, {
      ...
      let a = result.toJS();
      assetData[asset] = {
        symbol: a.symbol.replace(/OPEN\.|BRIDGE\.|RUDEX\.|GDEX\.|BLOCK\./, ""),
