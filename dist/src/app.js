...
async function doWork() {
  ...
  if (config.useES) console.log("Using Elastic Search for account history:", config.esNode, "\n");
  const {
    accountId,
    balances,
    assets
  ...
  while (true) {
    ...
    let result = config.useES ? await accountHistoryApi.getAccountHistoryES(accountId, pageSize, start, config.esNode) : await accountHistoryApi.getAccountHistory(accountId, stop, pageSize, start);
    ...
    if (config.useES) start += result.length;else start = opHistoryObject + (utils.getIndex(minSeen) - 1);
  }
